defmodule RaffleBot.Discord.Buttons.AdminPickWinner do
  @moduledoc """
  Handles the "Pick Winner" button click from admin thread.

  Selects a random winner using weighted selection (1 spot = 1 entry)
  and posts a preview to the admin thread with Confirm & Re-Roll buttons.
  """
  ...
  def handle(%Interaction{data: %{"custom_id" => "admin_panel_pick_winner_" <> raffle_id}} = interaction) do
  ...
  defp pick_winner(interaction, raffle) do
    case Raffles.select_random_winner(raffle.id) do
      ...
      {:ok, winning_claim} ->
        ...
        # Post winner preview to admin thread
